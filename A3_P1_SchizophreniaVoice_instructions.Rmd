...
```{r}
library(pacman)
pacman::p_load(tidyverse, purrr, plyr)
...
#Add 1000 to participant number for study 5 and 6 as these are not unique
demographic$Participant <- ifelse(demographic$Study == 5|demographic$Study == 6, demographic$Participant + 1000, demographic$Participant)
...
#Add 1000 to participant number for study 5 and 6 as these are not unique
pitch_data$Participant <- ifelse(pitch_data$Study == 5|pitch_data$Study == 6, pitch_data$Participant + 1000, pitch_data$Participant)
# Now we merge them
demo_pitch <- merge(demographic, pitch_data, by = c("Study", "Participant", "Diagnosis"))
demo_pitch_art <- merge(demo_pitch, art, by = "soundname")
art_pitch <- merge(art, pitch_data, by = "soundname")
art_pitch_demo <- merge(art_pitch, demographic, by = "Participant")
# Now we save them
...
data <- demo_pitch_art[complete.cases(demo_pitch_art),]
#Summary of data
